...
if($date == ""){
  $datesqlpcsc = "WHERE adid = '$UID'";
}else{
      $datesqlpcsc = "WHERE date = '$date' or date = 'daily' and adid = '$UID' ";
}
...
if($result->num_rows >0){
        while ($row = $result->fetch_assoc()) {
                ...
                echo '<div class ="boxdisp">';
                 ...
                 echo "<button  class='btn btn-primary btn-sm bokmobibt2 backred2' data-toggle='modal' data-target='#myModal'  onclick=(remove(this.id))   id = '".$row["id"]."'>Remove</button>" ;
                 echo "<button  class='btn btn-primary btn-sm bokmobibt2' data-toggle='modal' data-target='#myModal'  onclick=(editproduct(this.id))   id=$row[id]>Edit</button>" ;
                echo '</div>';
