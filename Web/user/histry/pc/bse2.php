...
$todeaget = date("Y-m-d") ;
$todeaget = " ".$todeaget;
$sql = "SELECT * FROM book WHERE date < '$todeaget' and usid = '$UID'";
...
if($result->num_rows >0){
        ...
        while ($row = $result->fetch_assoc()) {
               ...
               <button  class='btn btn-success btn-sm btn-sme ' data-toggle='modal' data-target='#myModal'  onclick=(editproduct(this.id))   id = '".$row["bid"]."'>Ticket</button>
                <button  class='btn btn-danger btn-sm btn-sme' data-toggle='modal' data-target='#myModal'  onclick=(remove(this.id))   id='".$row["bid"]."'>Delete</button>
